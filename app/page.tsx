...
export default function PaperShaderPage() {
  ...
  const [isMenuOpen, setIsMenuOpen] = useState(false); // Hamburger menü state'i
    ...
    <div className="w-full h-screen bg-black relative overflow-hidden">
      ...
      {activeEffect === "dots" && (
        <div className="w-full h-full absolute inset-0 bg-black">
          <DotOrbit className="w-full h-full" speed={speed} />
        </div>
      )}
      {activeEffect === "combined" && (
        <>
          ...
          <div className="w-full h-full absolute inset-0 opacity-60">
            <DotOrbit className="w-full h-full" speed={speed * 1.5} />
          </div>
        </>
      )}
          ...
          <header className="relative z-20 flex items-center justify-between px-6 py-4 max-w-7xl mx-auto">
            ...
            <div className="flex items-center">
              <img
                src="/logo.png"
                alt="Logo"
                className="w-20 h-8 object-contain transform translate-x-[-0.5px] sm:w-30 sm:h-10"
              />
            </div>
            ...
            {/* Navigation (Masaüstü: yatay, Mobil: dikey menü) */}
            <nav
              className={`${
                isMenuOpen ? "flex" : "hidden"
              } md:flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-2 md:ml-8 absolute md:static top-16 left-0 right-0 bg-black/95 md:bg-transparent p-4 md:p-0 z-10 pointer-events-auto`}
            >
              <Link
                href="/"
                ...
                onClick={() => setIsMenuOpen(false)} // Mobil menüyü kapat
              >
                Home / მთავარი
              </Link>
              <Link
            ...
            <div
              id="gooey-btn"
              className="hidden md:flex relative items-center group ml-8"
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="text-center font-mono">
          <div className="text-m text-white/50">
            We are preparing something new for you. Launching soon. / ჩვენ თქვენთვის ახალს
            ვამზადებთ. მალე გაიხსნება.
          </div>
